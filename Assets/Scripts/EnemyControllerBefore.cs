 ...
 */
public class EnemyControllerBefore : MonoBehaviour
{
    ...
    public float idleDistance = 0.02f;
    ...
    void UpdateIdle()
    {
        ...
        float hover = timePingPong * idleDistance; // Calculate the hover distance
        transform.Translate(0, hover, 0);

        // Change state if we've been Idling for long enough
        if (stateStartTime + time2 < Time.realtimeSinceStartup)
        {
            // Ground the enemy
    ...
    void UpdatePatrol()
    {
        ...
        Vector3 goalPosition = patrolNormal * distance;
        ...
        if (stateStartTime + time1 < Time.realtimeSinceStartup)
        {
